...
public class ChangeMatrixDeleteTransactionListener implements TransactionListener {
	...
	protected static final Logger LOGGER = Logging.getLogger(ChangeMatrixDeleteTransactionListener.class);
	...
	public void dataStoreChange(TransactionEvent event) throws WFSException {
		...
		{
			LOGGER.fine("ChangeMatrixDeleteTransactionListener - catched request for FeatureType: " + typeName);
		}
			...
			for (Object ft : features) {
				if (ft instanceof SimpleFeature) {
					...
					try {
						...
						if (LOGGER.isLoggable(Level.FINE))
						{
							LOGGER.fine("ChangeMatrixDeleteTransactionListener - checking feature: " + next.getID());
						}
						...
						if (LOGGER.isLoggable(Level.FINE))
						{
							...
							LOGGER.fine("ChangeMatrixDeleteTransactionListener - layerName: " + layerName);
						...
						if (wsName != null && itemStatus != null
							...
							if (ws != null) {
								...
								storeInfo = catalog.getDataStoreByName(layerName.trim()) != null ? catalog.getDataStoreByName(layerName.trim()) : catalog.getDataStoreByName(ws.getName(), layerName.trim());
								storeInfo = (storeInfo == null ? (catalog.getCoverageStoreByName(layerName.trim()) != null ? catalog.getCoverageStoreByName(layerName.trim()) : catalog.getCoverageStoreByName(ws.getName(), layerName.trim())) : storeInfo);

								if (storeInfo == null) {
									LOGGER.severe("Could not retrive Store " + layerName + " into the Catalog");
								}
								else
								{
								        ...
								        for (ResourceInfo ri : resources) {
								            ...
								            if (!layers.isEmpty()){
								                for (LayerInfo li : layers) {
								                	...
								                	if (LOGGER.isLoggable(Level.FINE))
													{
														LOGGER.fine("ChangeMatrixDeleteTransactionListener - going to check layer: " + li.getName() + " against feture layer attribute: " + layerName);
													}
								            ...
								            try {
								            	...
								            	LOGGER.info("ChangeMatrixDeleteTransactionListener - ConnectionParameters URL: " + ((CoverageInfo)ri).getStore().getURL());
								            	...
								            	if (storeResource != null && storeResource.exists() && storeResource.isFile() && storeResource.canWrite()) {
								            		...
								            		for (; tryDelete < 5; tryDelete++)
								            		{
								            			...
								            			else
								            			{
								            				if (LOGGER.isLoggable(Level.FINE))
								            				{
								            					LOGGER.fine("ChangeMatrixDeleteTransactionListener - could not delete storeResource: " + storeResource.getAbsolutePath() + " ... wait 5 secs and retry...");
								            				}
							...
							if (file.exists() && file.isFile() && file.canWrite()) {
								...
								for (; tryDelete < 5; tryDelete++)
								{
									...
									else
									{
								        if (LOGGER.isLoggable(Level.FINE))
										{
											LOGGER.fine("ChangeMatrixDeleteTransactionListener - could not delete file: " + srcPath + " ... wait 5 secs and retry...");
										}
								...
								if (tryDelete > retries)
								{
									LOGGER.severe("ChangeMatrixDeleteTransactionListener - Could not delete file "+srcPath+" from the FileSystem");
								}
							}
						}
						...
						if (soundVelocityProfile != null && soundVelocityProfile.length() > 0) {
							...
							if (file.exists() && file.isFile() && file.canWrite()) {
								...
								if (tryDelete > retries)
								{
									LOGGER.severe("ChangeMatrixDeleteTransactionListener - Could not delete file "+soundVelocityProfile+" from the FileSystem");
								}
							}
						}
					} catch (Exception e) {
						LOGGER.severe("ChangeMatrixDeleteTransactionListener - Exception occurred during Deletion: "+e.getLocalizedMessage());
